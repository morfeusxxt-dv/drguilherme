...
const proc = d.document.children[0].children.find(c => c.name === 'Guilherme Cunha - Cirurgias - Desktop');
function printNode(n, indent='') {
    let out = indent + n.name + ' (' + n.type + ')\n';
    if(n.characters) out += indent + '  "' + n.characters.replace(/\n/g, '\\n') + '"\n';
    fs.appendFileSync('proc_structure.txt', out);
    if(n.children) n.children.forEach(c => printNode(c, indent+'  '));
}
fs.writeFileSync('proc_structure.txt', '');
printNode(proc);
